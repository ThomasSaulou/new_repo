    ...
    , ui(new Ui::MainWindow)
{
    ...
    //connect(ui->btn_, SIGNAL(valueChanged(int)), this, SLOT(on_sizeslider_valueChanged(int)));
...
void MainWindow::Mouse_current_pos()
{
    ...
    if (m_mouseVal==2){
        if (m_mouseClick==1){
            if(y<(h+imh)/2 && y>(h-imh)/2){
                for (int xblue=-int(sqrt(m_SelectSize));xblue<int(sqrt(m_SelectSize));xblue++){
...
void MainWindow::on_btn_import_clicked()
{
    ...
    imName= filename.toStdString().c_str();
    fileN = filename.toUtf8().constData();
...
void MainWindow::on_btn_Perspective_clicked()
{//const char * filename, const char * window_name
   // cout<< *imName <<endl;
    a=1;
    user_interface::window_handler window_handler(imName, "Perspective Manipulation");
 while (a==1){
    handle_shortcut(static_cast<char>(cv::waitKey(0)));
    window_handler.handle_shortcut(static_cast<char>(cv::waitKey(0)));
 }
 a=1;

}

void MainWindow::handle_shortcut(char key_code){
    if (key_code=='z')
        a=0;
}
...
void MainWindow::on_btn_seam_clicked()
{
    seam seamcarving(this, fileN);
    seamcarving.setModal(true);
    seamcarving.exec();

}

void MainWindow::on_btn_harmonization_clicked()
{
